...
def avgby(tbl, col, by):
    ...
    avgdict = {}
    for row in tbl:
        if row[by] in avgdict:                  #if by number is already in dictionary
            avgdict[row[by]].append(row[col])   #append by numbers column to dict values
        else:
            avgdict[row[by]] = [row[col]]       #add by to key of dict and col num to value

    for k, v in avgdict.items():                #for keys and values in dictionary
        avgdict[k] = (sum(v)) / len(v)      #add values and divide by total # of values to get avg

    return avgdict
# Theorem of solverem
# Place the by as keys of dictionary
# Place the col as values of dictionary
# If keys is repeated just append the value
# Append final values to keys
...
def merge(left, right, left_on, right_on):
    merged = []
    for a in left:  # for element in list one
        for b in right:  # for element in list two
            if a[left_on] == b[right_on]:  # if both elements at each indice equal
                merged.append(a + b)        #append to merged list
    return merged
